...
// The map used to store the query times, where the query unique identifier is the key.
const times: {
  [key: string]: {
    startTime: number;
    endTime?: number;
...
const printQueryWithTime = (uid: string) => {
  ...
  const elapsedTime = endTime || 0 - startTime;

  const logColor = elapsedTime > 20 ? 'red' : 'blue';

  // I print the sql generated for a given query, as well as
  // the bindings for the queries.
  if (!query.sql.includes('select `emails`.*')) {
    log.debug(
      color[logColor](`[QUERY]<${elapsedTime.toFixed(3)} ms>: `),
      query.sql,
      ',',
...
const queryLog = () => {
  if (app.logging.debugSQL) {
    knex
      ...
      .on('query-response', (_response: unknown, query: QueryContext) => {
        const uid = query.__knexQueryUid;
