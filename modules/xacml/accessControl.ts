import transform from './accessControllang';
import XacmlHelper from './XacmlClass';
import { Construct } from './types';
import { NextFunction, Request, Response } from 'express';

export default ({ pre = [], validation, secondaryValidations }: Omit<Construct, 'preRequest'>) =>
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { body, params, query, user, file, files, headers } = req;
